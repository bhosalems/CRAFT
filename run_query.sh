...
_pool_wait_gpu_free() {
	...
	for ((i=0; i<POOL_GPU_FREE_TIMEOUT; i++)); do
		...
		for phys in $phys_list; do
			used=$(nvidia-smi --query-gpu=memory.used --format=csv,noheader,nounits -i "$phys" 2>/dev/null | tr -d ' ')
...
# dev branch TODO: Update this later.
QUERIES_JSONL="/a2il/data/mbhosale/MAGMaR2026_test/MAGMaR2026_queries_dev.jsonl"
MAPPING_JSON="data/topic_video_mapping_dev.json"

# Video root where <video_id>.mp4 files live (also where chunks are written).
VIDEO_ROOT="/a2il/data/mbhosale/MAGMaR2026_test"
...
# Usage:
#   ./run_query.sh <OUT_DIR> [QUERIES_JSONL] [MAPPING_JSON] [CLAIMS_JSONL]
# Examples:
#   ./run_query.sh outputs_query_branchv5
#   SKIP_STEP1=1 ./run_query.sh outputs_query_branchv5 /path/to/queries.jsonl
#   ./run_query.sh outputs_query_branchv5 /path/to/queries.jsonl data/topic_video_mapping.json /path/to/query_conditioned_claims.jsonl
OUT_DIR="${1:-outputs_query_branchv5}"
...
ASR_DIR="${ASR_DIR:-${VIDEO_ROOT}/asr}"
