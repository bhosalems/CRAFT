...
## 11. Caching, idempotency, parallelism summary

| Property | Behavior |
|---|---|
| Stage 0 chunks | written once; cache hit on file presence |
| Stage 0.5 ASR | per-video JSON; force re-run with `--force` |
| Stage 0.7 AKS | external; resolver returns the AKS path when present |
| Stage 1b extraction | per-query JSONL with resume; recombine pass produces combined file |
| Stage 1.5 UNLI | predictions cached per claim; calibration stateless |
| Stage 2a packets | stateless |
| Stage 2b inference | per-query JSONL; silent fallback when JSON parse fails |
| Stage 3 report | stateless |
| Submission | stateless |

`run_query.sh` is the single orchestrator. It detects partial state
(`SKIP_CHUNK=auto`, `SKIP_STEP1=auto`) and only re-runs what's
missing. Parallelism is **only at Stage 1b**, gated by
`PARALLEL_QUERIES`.

---

## 12. Mapping to MIRAGE failure modes — quick reference

| MIRAGE axis | Components that target it |
|---|---|
| \textsc{InfoP-Ref} (precision against reference text) | Stage 1b atomic-claims rule, Stage 1b critic loop (UNLI temporal grounding), Stage 2b atomic-inference rule, Stage 2b no-elaboration rule, AKS frame curation |
| \textsc{InfoR} (recall against reference text) | Stage 1b coverage-friendly relevance, Stage 1b coverage audit + follow-up, Stage 1b topic fallback, Stage 0.5 ASR signal, Stage 0.5 translation pass |
| \textsc{CiteP} (citations are supported) | Stage 1b critic UNLI gate, Stage 1.5 calibration, Stage 2b citation-merging dedup |
| \textsc{CiteR} (citations cover GT references) | Stage 1b coverage-friendly relevance (every video stays visible), Stage 2b citation-merging (no source_id is dropped), Stage 0.5 ASR (gives the model an alternative grounding signal so it doesn't reject otherwise-relevant videos) |

---

## 13. Models in use (single source of truth)

| Stage | Model | Purpose |
|---|---|---|
| 0.5 (ASR) | Qwen3-ASR-1.7B | primary speech recognition |
| 0.5 (ASR fallback) | openai/whisper-large-v3 | low-resource languages, translation |
| 1b (VLM) | Qwen3.5-9B (Qwen3-VL-32B-AWQ alt) | claim extraction |
| 1b (critic temporal) | Qwen2.5-Omni-3B + LoRA `AdoptedIrelia/UNLI` | per-claim video-claim entailment |
| 1b (critic contradiction screen) | DeBERTa-v3 MNLI | claim-pair contradiction screening |
| 1b (critic adjudicator + coverage) | meta-llama/Llama-3.2-3B-Instruct | confirms contradictions, audits coverage |
| 1.5 | same UNLI model | calibrated per-claim probability |
| 2b (inference LLM) | Qwen3.5-9B (text-only) | atomic consolidation |
| Evaluator | Qwen2.5-7B-Instruct (MIRAGE judge) | per-claim entailment for MIRAGE scoring |

---

## 14. Suggested figure layout

For a single-column ACL methods figure, the natural grouping is:

- **Box 1 — Preprocessing.** Chunking, ASR (Qwen3-ASR + Whisper +
  loop detector + translation), AKS. Annotate: "all caches keyed by
  video_id and reusable across runs".
- **Box 2 — Extraction (Stage 1b).** VLM call, with inset for the
  hybrid critic (UNLI / MNLI / LLM-adjudicator) and the topic
  fallback. Annotate: "8-way query parallelism; per-video critic
  loop; persona+ASR+visual prompt".
- **Box 3 — Calibration (Stage 1.5).** UNLI bulk scoring + calibration
  arrow into Stage 2a.
- **Box 4 — Synthesis (Stages 2b + 3).** Atomic inference with
  citation-merging; report assembly with end-to-end provenance arrow
  back to Stage 1b citations.
- **Box 5 — Submission.** Chunk-id remap + final JSONL.
- **Right margin** — vertical color-bar showing which MIRAGE axis
  each box primarily targets (\textsc{InfoP-Ref}, \textsc{InfoR},
  \textsc{CiteP}, \textsc{CiteR}).
