        ...
        return [
            ...
            },
            {
                ...
                "content": [
                    {
                        "type": "text",
                        "text": INSTRUCTION_TEXT + "\n\n" + PROMPT_TEXT.format(sentence=sentence, claim=claim),
                    }
                ]
            }
        ]
...
class OmniEvaluator:
    ...
    def create_model(self):
        ...
        self.model = Qwen2_5OmniThinkerForConditionalGeneration.from_pretrained(self.config.processor_path, torch_dtype="auto",device_map=self.config.device,attn_implementation="flash_attention_2",)
    ...
    def process_data(self, llm_inputs: list[dict[str, any]]) -> list[dict[str, any]]:
        ...
        if self.modality == 'text':
            ...
            try:
                ...
                inputs = inputs.to(self.model.device).to(self.model.dtype)
        with torch.no_grad():
            outputs = self.model.generate(**inputs,use_audio_in_video=USE_AUDIO_IN_VIDEO)
            outputs = self.processor.batch_decode(outputs, skip_special_tokens=True,clean_up_tokenization_spaces=False)
        return outputs
    ...
    def evaluate(self):
        ...
        unfolded_data = self.filter_data(output_results, unfolded_data[self.config.data_begin:self.config.data_end])

        for i in tqdm(range(0, len(unfolded_data), self.config.batch_size)):
            llm_inputs = []
            batch_data = unfolded_data[i:i+self.config.batch_size]
            ...
            outputs = self.process_data(llm_inputs)


            for batch_data, output in zip(batch_data, outputs):
                answer = output
                if self.modality == 'text':
...
if __name__ == "__main__":
    ...
    if config.modality == 'video' or config.modality == 'omni':
        ...
        os.makedirs(os.path.join(config.output_eval_path), exist_ok=True)
        config.output_path = os.path.join(config.output_eval_path,  f'{config.dataset_name}_{config.modality}.json')
