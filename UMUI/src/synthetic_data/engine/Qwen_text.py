...
class QwenTextHFInferenceEngine: # text only
    def __init__(self, config: AppConfig) -> None:
        from transformers import AutoModelForCausalLM, AutoTokenizer
        self.modality = config.modality
        self.config = config
        self.model = AutoModelForCausalLM.from_pretrained(config.model, cache_dir=config.cache_dir, device_map="auto")
...
class QwenTextVLLMInferenceEngine: # text only
    def __init__(self, config: AppConfig) -> None:
    ...
    def generate_batch(self, batch_data: list[dict[str, any]]) -> list[str]:
        ...
        for claim, sentence in zip(claims, sentences):
            ...
            messages.append(self.tokenizer.apply_chat_template(message, tokenize=False, add_generation_prompt=True))
